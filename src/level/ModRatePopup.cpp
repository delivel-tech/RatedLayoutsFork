...
void ModRatePopup::onInfoButton(CCObject* sender) {
      ...
      postTask.listen([this](web::WebResponse* response) {
            ...
            std::string infoText = fmt::format(
                ...
                "Featured Score: {}",
                levelId, averageDifficulty, suggestedTotal, suggestedFeatured, featuredScore);
...
void ModRatePopup::onSubmitButton(CCObject* sender) {
      ...
      postTask.listen([this](web::WebResponse* response) {
            ...
            if (!response->ok()) {
                  log::warn("Server returned non-ok status: {}", response->code());
                  Notification::create("Failed to submit layout",
                                       NotificationIcon::Error)
                      ->show();
                  return;
            }
            ...
            if (success) {
                  ...
                  if (existingData) {
                        ...
                        if (parsed) {
                              ...
                              auto writeResult = utils::file::writeString(cachePath.string(), jsonString);
                              log::debug("Deleted level ID {} from cache after submission", this->m_levelId);
                        }
                  }
            ...
            if (success) {
                  ...
                  if (existingData) {
                        ...
                        if (parsed) {
                              ...
                              auto jsonString = root.dump();
                              auto writeResult = utils::file::writeString(cachePath.string(), jsonString);
                              log::debug("Deleted level ID {} from cache after unrate", this->m_levelId);
                        }
                  }
